...
SECRET_KEY = os.getenv("SECRET_KEY", "fixed_key_2026_final_v2")
ALGORITHM = "HS256"
...
class Settings(Base):
    ...
    show_git_commit = Column(Boolean, default=True) # 新增开关
...
def init_db():
    ...
    try:
        # 字段自动迁移逻辑
        conn = engine.connect()
        # 修复 folder_id 和 git_commit 字段 (之前已做)
        try: db.execute(text("UPDATE files SET folder_id = 0 WHERE folder_id IS NULL"))
        except: pass
        # 新增：show_git_commit 开关迁移
        try: db.execute(text("ALTER TABLE settings ADD COLUMN show_git_commit BOOLEAN DEFAULT 1"))
        except: pass
        db.commit()
...
def get_data(db: Session = Depends(get_db)):
    ...
    files = [{
        ...
        "git_commit": f.git_commit, "submitter": f.submitter, 
        "date": f.upload_date.strftime("%Y-%m-%d %H:%M"), "folder_id": f.folder_id or 0
    } for f in files_db]
    folders = [{"id": f.id, "name": f.name, "parent_id": f.parent_id or 0, "creator": f.creator} for f in db.query(Folder).all()]
...
async def upload(file: UploadFile = File(...), version: str = Form(""), changelog: str = Form(""), git_commit: str = Form(""), folder_id: int = Form(0), user: User = Depends(get_user), db: Session = Depends(get_db)):
    ...
    db.add(FileRecord(filename=file.filename, filepath=save_path, md5=hashlib.md5(content).hexdigest(), version=version, changelog=changelog, git_commit=git_commit, submitter=user.username, folder_id=folder_id))
    db.commit(); return {"status": "success"}
